...
const getFriends = async (req, res) => {
    ...
    const user = await User.findById(req.params.id)
    const uf = await user.populate('friends');
    res.send({
      error: false,
      message:`All friends from the User with the id  ${req.params.id} `, 
      friends: uf.friends
    });
...
//koge ke otidete na rutata /:id/addFriend 
// treba da se pushne idto na toj shto saka da go dodade vo nizata "friends" vo bazata na 1 user
//treba site useri od baza prvo da se izbrishat